...
- 每条固定三行结构，用大白话，禁止贴大段代码：
  1. **面临的选择**：列出当时真实存在的 2-3 个候选方案（含被否决的）。
  2. **选了哪个 + 为什么**：选定项与决定性理由（职责边界 / 一致性 / 风险 / 与既有基线兼容等）。
  3. **放弃的代价**：被否决方案各自的具体代价，即"若选它会发生什么糟糕的事"。
- out_of_scope 的边界决策（"刻意不做什么、为什么"）也必须各记一条。
- 决策必须是本 change 真实发生的取舍；不确定的写"待用户在事前闸门确认"，不得编造备选方案凑数。
- 本节只写进 `design.md`，不写进 `.ai/AGENT_LOG.md`；AGENT_LOG 保持纯执行证据。
- 完整措辞模板见 `docs/workflow/prompt-snippets/design-decision-record.md`。
