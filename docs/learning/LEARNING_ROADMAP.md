...
- 当前默认是 `HeuristicReranker`，同时已有默认关闭的 HTTP `ModelReranker` adapter；要能解释两者差异以及为什么真实模型收益尚未验证
...
后续每周任务应通过统一的 Agent 日志记录进度；涉及代码或契约变化的任务，在引入 OpenSpec 后进入独立 change，避免继续生成新的平行总计划。
