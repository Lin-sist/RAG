...
### 9.4 C8b expanded v2（待用户验收）

C8b 已构建但尚未设为默认的 `rag-eval-dev-v2`：`docs/eval/releases/rag-eval-dev-v2.jsonl` 共 150 条，前 30 条与 v1 seed 的对象、顺序及原始行 bytes 一致，后续追加 120 条。sample schema 和 3 份 fixture 未改变，因此继续使用 `rag-eval-sample-v1` 与 `fixtures-v1`；question、annotation 和 release 分别升级为 `questions-v2`、`annotations-v2`、`rag-eval-dev-v2`。
...
验收前只能显式选择 v2，默认 `docs/eval/dataset-manifest.json` 仍保持 v1：
...
`docs/eval/releases/rag-eval-dev-v1-manifest.json` 与 v1 question set 仍可独立验证。只有用户验收 expanded data、review evidence 和 release identity 后，才可把默认 manifest 切到 v2、接受 C8b spec delta 并归档；v2 冻结后任何题目、标注、quota、review 或 corpus 修订都必须创建后续新 version，不能根据 provider/算法结果原地改写。
...
- 当前默认、已验收 baseline 仍是 30 条 v1；150 条 v2 已构建但待用户验收和默认切换。两者都只适合开发评测，不是最终论文级评测。
