...
- `Objective metric status: COMPLETE / PARTIAL / RETRIEVAL_ONLY / FAILED`
- `Judge metric status: COMPLETE / PARTIAL / SKIPPED / NOT_APPLICABLE`
- objective/judge 各自的 `comparisonSafety`
- `askErrors count`
...
- `claimMetricConfig`：固定 `claim-lexical-v1`、splitter/tokenizer 版本、`0.70` 阈值、最少 2 token 与 evidence policy。
...
- `PARTIAL`：可能来自 objective 缺口，也可能仅来自 judge 缺口；必须查看两个 channel status，不能再根据全局值推断“只剩 retrieval 可比较”。
...
| objective lexical claim support rate | 所有成功抽取的 answerable claims 中，能与 provenance-valid returned citation snippet exact/token 对齐的比例 | `ask` | 答案里有多少可审计 claim 具备显式词法证据对齐 |
| objective claim metric status | `COMPLETE / PARTIAL / SKIPPED / NOT_APPLICABLE` | `ask` | claim 通道的分母是否完整，还是存在 ask/抽取缺口或本轮不适用 |
| unsupported claim count | 没有 eligible evidence、token 太短或低于 `0.70` coverage 的 claim 数量 | `ask` | 哪些生成内容缺少可复现的客观词法证据 |
...
注意：answer keyword hit rate 是字符串关键词匹配，不等价于完整答案正确率。citation snippet hit rate 只验证片段可回连。C9a objective lexical claim support 会进一步检查生成 claim 与已验证 citation snippet 的 exact/token 对齐，但仍不等价于语义蕴含、完整事实正确性或独立 faithfulness judge。

C9a 的固定算法身份如下，并同时写入 plan、Markdown report、details JSON 与 reproducible run metadata：
...
只有通过现有 citation identity + snippet-to-context provenance 校验的 returned citation 才能成为 claim evidence；未引用的 retrieved context 不会被用来抬高 support rate。无 evidence 或未匹配 claim 仍保留在分母。no-answer 继续走独立指标，不进入 claim 分母。不同 claimMetricConfig 的报告不得直接比较；旧报告缺少 C9a 字段时应解释为 `unavailable`，不能补算为 0。

### 6.1 C9b judge contract、calibration 与状态语义

C9b 已验收归档，固定唯一 `rag-judge-v1` contract；normal eval 与独立 calibration runner 共用 system prompt、strict JSON parser、`0.70` faithfulness/relevance 双阈值和 score-derived joint pass。score 缺失、字符串、NaN/Infinity、越界、非法 JSON 或 schema 缺口统一成为 `invalid_judge_payload`，不得 clamp 或进入质量分母；provider 自报 `pass` 只保留为 disagreement 诊断。

静态 `judge-calibration-v1` 与 v1/v2 评测 release 完全分离：24 条人工复核 case 覆盖 faithful×relevant 四象限各 6 条，只使用三份 tracked fixture 的 exact excerpt，不调用 backend、embedding、rerank、ask 或 generation。离线查看预算和身份：

```powershell
python -B scripts\run_judge_calibration.py --plan-only --profile canary --judge-model <model>
python -B scripts\run_judge_calibration.py --plan-only --profile full --judge-model <model>
```

- canary 固定 4 cases × 1 repeat，最多 4 次 judge 调用；
- full 固定 24 cases × 3 repeats，最多 72 次 judge 调用；
- 无自动 retry，失败 observation 和缺口必须留在 coverage 分母；
- aggregate 只输出 confusion/agreement、parse coverage、provider-pass mismatch、repeat consistency 和安全错误分类；
- raw details 默认写入 `tmp/eval/`，执行时应加 `--no-overwrite`，不得默认纳入 tracked evidence；
- 真正执行还必须显式添加 `--execute-live-judge`，但该开关不替代协作层的单独外调授权。

在获得单独授权前，不得运行 live canary/full。授权披露需固定 provider/model/endpoint/temperature/timeout、Git HEAD、manifest 与 contract identity，并说明 tracked question、synthetic answer、fixture excerpt 的数据出站、费用和限流风险。C9b 不自动搜索 threshold、不输出 production go/no-go；这属于 C10。
...
C9a 已在 eval runner 中增加离线、确定性的 claim-evidence 客观指标：

1. 按段落、列表项和中英文句末标点拆分生成答案，不调用 LLM/NLP provider。
2. 只使用通过 provenance 校验的 returned citation snippet。
3. 先做 normalized exact，再以 claim token 为分母做固定 `0.70` ASCII/CJK-bigram coverage。
4. 输出逐 claim `exact / token_overlap / unsupported`、稳定 reason、best evidence 与 aggregate support rate。
5. ask 失败、空答案或抽取失败会把 claim 通道标成 `PARTIAL`；不会静默缩小分母后仍称完整。

C9a/C9b 只改离线评测工具，不修改 Java 生成、生产 prompt、citation validator、重答或生产返回。C9b 的 offline implementation 已验收归档，但 live calibration 按 `SKIPPED` 收口且未来仍需单独授权；对 unsupported claim 做线上标记/重答及阈值/退出码门禁仍需独立 Type C change，后者属于 C10。
