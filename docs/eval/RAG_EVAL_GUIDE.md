...
C9b offline implementation 固定唯一 `rag-judge-v1` contract，normal eval 与独立 calibration runner 共用 system prompt、strict JSON parser、`0.70` faithfulness/relevance 双阈值和 score-derived joint pass。score 缺失、字符串、NaN/Infinity、越界、非法 JSON 或 schema 缺口统一成为 `invalid_judge_payload`，不得 clamp 或进入质量分母；provider 自报 `pass` 只保留为 disagreement 诊断。
...
C9a/C9b 只改离线评测工具，不修改 Java 生成、生产 prompt、citation validator、重答或生产返回。C9b 当前只有 offline implementation，live calibration 仍需单独授权；对 unsupported claim 做线上标记/重答及阈值/退出码门禁仍需独立 Type C change，后者属于 C10。
