...
- [x] RED：header/query/body/cookie/metadata selector、缺失/非法 tenant claim 和 foreign IDs 取得预期失败；C14 IT 与 `JwtTokenProviderPropertyTest` 聚焦验证均通过。
...
- [x] 对 KB/task foreign 与 matched nonexistent control 比较 status/error/schema fingerprint，禁止 foreign identity/content；26-case evidence 全部完成。
- [x] 跨 tenant KB/document/task/history/feedback 写入口验证 tenant B SQL row、版本、删除标记及相关状态前后不变。
...
- [x] 覆盖 auth session、QA/embedding cache、idempotency、task projection/payload mismatch 与旧无 tenant key 不回退；mapped Surefire 与相邻边界聚焦 suites 通过。
- [x] 覆盖 Redis projection miss、system scan、claim/heartbeat/phase/finalize/recovery/cancel，确认 scope 从 durable ledger 重建。
- [x] 覆盖 input open/delete/cleanup、业务 lock 与 path traversal/symlink 组合；`FileSystemIndexInputStoreTest`、`DocumentServiceDurableInputTest` 与既有 cleanup/lock contract 证明跨 tenant fail closed。
- [x] 验证 token blacklist/global IP rate limit 保持 global security 语义；相邻聚焦 suite 通过，且未把它们计入 26-case tenant business cache evidence。
...
- [x] `VectorStoreTenantEnforcementGuardTest` 复核 Qdrant/Elasticsearch enforcement-mode fail startup；正式 report 明确不把 Milvus evidence 外推。
...
- [x] details/summary/evidence 只输出 case ID、bounded taxonomy、hash/计数与聚合；raw token/body/content/canary/用户绝对路径扫描 0 命中。
...
- [x] 运行 case driver 与 gap-fix 聚焦 tests：mapped Surefire 71/0/0/0，C14 Failsafe 5/0/0/0，相邻边界 58/0/0/0。
...
- [x] 运行 `python -B -m unittest discover -s scripts -p 'test_*.py'`；190 tests 通过，既有 evaluation contract/quality baseline 未漂移。
- [x] 前端无改动，正式 build 按约定记为 `SKIPPED`；无 DTO 改动。
