...
### 决策：2026-09-08 full 本地限流恢复
- **面临的选择**：降低评测请求频率；关闭服务端限流；拼接失败执行的成功样本。
- **选了哪个 + 为什么**：增加显式 retrieval delay，正式新执行固定1.2秒/样本，低于既有60次/60秒上限；计入metadata/strict repeat identity，等待排除在单次检索延迟之外。用户已明确授予阶段内执行权限，不反复请求同一数据目的地授权；每次新执行使用独立no-overwrite身份且内部retry=0。
- **放弃的代价**：关闭限流会改变运行保护边界；拼接成功样本会破坏exact450与完整repeat身份。失败full保留，重新执行全部150×3，不清理缓存或KB。
