...
建议初始默认：scan batch 20、concurrency 1、lease 5 分钟、heartbeat 60 秒、max attempts 3、指数 backoff；均为待用户确认的配置值，不写死为不可变契约。
...
2. **选了哪个 + 为什么**：建议选 B，待用户确认；既有 MySQL schema 可前向扩展，能支持 phase/lease/CAS，且 Redis outage 正是需要协调的场景。
...
2. **选了哪个 + 为什么**：建议选 B，待用户确认；恢复需要可查询 phase/lease，但输入 bytes 已由 C5a 管理，不应复制敏感内容。
...
2. **选了哪个 + 为什么**：建议选 B，待用户确认；客户端拿到 taskId 前必须同时具备 durable command facts 和现有轮询投影，已知失败不能启动 operation。
...
2. **选了哪个 + 为什么**：建议选 C，待用户确认；DB 已是 durable ledger，可用同一事实源原子 claim，并避免 Redis outage 时失去协调依据。
...
2. **选了哪个 + 为什么**：建议选 B，待用户确认；它能恢复可证明安全的窗口，同时遵守 C4d outcome unknown 不自动 replay。
...
2. **选了哪个 + 为什么**：建议选 B，待用户确认；相同 document/content/config 可重建相同 IDs，且无需在 ledger 复制正文或向量。
...
2. **选了哪个 + 为什么**：建议选 C，待用户确认；外部 mutation 与 DB checkpoint 不能原子提交，in-flight 只能诚实表达结果不确定。
...
2. **选了哪个 + 为什么**：建议选 B，待用户确认；保留低延迟投影，同时让 TTL 不再抹掉已接受任务身份。
...
2. **选了哪个 + 为什么**：建议选 B，待用户确认；分类/cleanup 不产生模型费用，而自动 resume 可能导致数据出站和批量 embedding，必须显式授权。
...
2. **选了哪个 + 为什么**：建议选 B，待用户确认；旧状态不能证明 vector mutation 是否发生，保守隔离符合 C4d/C5a 边界。
...
2. **选了哪个 + 为什么**：建议选 B，待用户确认；这是 C5a 明确留给后续协调的安全副作用，且不需要 provider/vector 调用。
...
2. **选了哪个 + 为什么**：建议选 B，待用户确认；模板当前无 durable broker 且默认 consumer 不执行真实索引，复用会制造错误能力声明。
...
2. **选了哪个 + 为什么**：建议选 B，待用户确认；避免用户绕过 mutation 安全边界，也不扩大到前端重做。
