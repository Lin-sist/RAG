...
- [x] 确认 prod 精确拒绝集合及稳定错误类别。
- [x] 在 `rag-auth` 新增可纯函数测试的 JWT production secret guard/policy。
- [x] 通过显式依赖保证 guard 在 JJWT key 构造前执行。
- [x] 保持非 prod fallback 与现有 UTF-8/JJWT key-strength 职责不变。
- [x] 更新 tracked `README.md` 的生产配置说明；未修改 `.env.local` 或任何真实 secret。

## Phase 2：测试与验证

- [x] 添加 prod known default、blank、首尾空白、未解析占位符和合法 secret 单元测试。
- [x] 添加错误消息不泄露 secret 的断言。
- [x] 添加非 prod 兼容与 JJWT UTF-8 byte-length 职责回归测试。
- [x] 添加最小 Spring context 的 prod fail-fast / valid-secret 测试，不启动业务外部依赖。
- [x] 运行 `mvn -q -pl rag-auth -am test`。
- [x] 运行 `mvn -q test`。
- [x] 运行 Markdown 相对链接/旧路径扫描与 `git diff --check`。
- [x] 确认未发生 embedding、rerank、judge、ask 或其他 provider 业务调用。
