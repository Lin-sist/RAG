...
这里的“单样本”指一次 debug/eval retrieval 的 NVIDIA ranking 请求。同步 QA 若命中既有 explanatory fallback，会在 metadata 中累计多个独立 retrieval 的真实 model call 数，因此总数可以大于 1；这不是 provider 自动 retry。
...
### 决策 13：explanatory fallback 首个非空结果即停止
- **面临的选择**：继续合并多个 fallback query 的已重排 contexts；只采用最后一个非空结果；采用首个非空结果并停止后续 fallback query。
- **选了哪个 + 为什么**：实现中选择首个非空结果即停止；这样实际生成 contexts 只对应一个 effective provider，不制造 model/heuristic 混合排序，同时减少额外 provider 调用。同步 QA metadata 采用该次 retrieval 的 attribution，并累计初始检索与 fallback 检索的真实 model call/fallback/latency 数。
- **放弃的代价**：继续合并会让 final order 无法归因到单一 provider；只取最后结果会无谓增加调用且丢弃更早已找到的证据；首个命中停止可能错过后续 fallback query 的补充证据，但 QueryEngine 内部 query variants 已承担多变体召回。用户已于 2026-07-19 验收确认该实现期取舍。
