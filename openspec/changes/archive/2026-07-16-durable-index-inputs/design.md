...
2. **选了哪个 + 为什么**：选 A；用户已批准。当前没有业务对象存储 adapter，A 能以最小基础设施面建立 durable/reopen 契约，同时保留 `IndexInputStore` 扩展点。
...
2. **选了哪个 + 为什么**：选 B；用户已批准。它不伪造可恢复性，也不破坏已完成文档。
...
2. **选了哪个 + 为什么**：选 B；用户已批准。durable 保证必须由部署显式承担，错误挂载应 fail fast。
...
2. **选了哪个 + 为什么**：选 B；用户已批准。先让恢复材料可信，再单独审查重放的幂等与 outcome unknown 风险。
...
2. **选了哪个 + 为什么**：选 B；用户已批准。测试基础设施不等于业务 provider，首版应避免新增网络与凭据契约。
...
2. **选了哪个 + 为什么**：选 B；用户已批准。C4d 已证明 mutation 可能 outcome unknown，未经 C5b 对账与租约设计不能安全重放。
