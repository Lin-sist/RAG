...
- `confirmed`：上传入口已改为先通过 `IndexInputStore` 原子发布输入，再保存 document 与提交任务；异步任务只捕获稳定 ID/key/校验事实并重新打开 stream。
- `confirmed`：`document.file_path` 现保存 opaque storage key，V7 新增 nullable size/SHA-256/input state；`content_hash` 继续只承担解析后内容去重。
...
- `confirmed`：filesystem store 已实现 root confinement、staging + atomic move、大小/容量拒绝、size/SHA-256 校验、幂等删除与启动 fail-fast。
- `confirmed`：输入状态已区分 AVAILABLE/CLEANUP_PENDING/CLEANED/MISSING/CORRUPT，成功、失败、outcome unknown 与 canonical delete 生命周期已有测试证据。
- `confirmed`：`Document` 的 storage key、size、SHA-256 与 input state 已加序列化保护，现有文档列表不会把内部输入事实返回客户端。
- `out_of_scope`：孤儿扫描、lease/claim、自动重放、恢复调度、跨存储对账、对象存储 provider、公开 API/前端改造。
- `partial`：生产环境仍需由部署方把 `RAG_INDEX_INPUT_ROOT` 指向真实持久卷；失败输入保留期限与自动协调留给 C5b。
...
- 用户完成实现验收前不接受 C5a delta 进入 baseline，也不启动 C5b。
