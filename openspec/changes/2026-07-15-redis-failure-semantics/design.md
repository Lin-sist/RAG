...
- **选了哪个 + 为什么**：草案选择 consumer 分级并待用户批准，因为同一 Redis 同时承载可丢缓存和安全关键状态，统一语义必然产生可用性或安全错误。
...
- **选了哪个 + 为什么**：草案选择 fail-open 并待用户批准，因为缓存不是答案或 embedding 的事实源，失败只应损失性能。
- **放弃的代价**：outage 期间 provider/检索调用量和延迟会上升，需要安全降级日志识别。

### DR-3：鉴权与黑名单故障放行还是拒绝

- **面临的选择**：blacklist/session unknown 时按有效状态继续，或 fail-closed 返回 503。
- **选了哪个 + 为什么**：草案选择 fail-closed 并待用户批准，因为把 unknown 当作未撤销会让 logout 与 token revocation 失效。
...
- **选了哪个 + 为什么**：草案选择所有当前受限入口 fail-closed 并待用户批准，因为它们包含 login、问答和写操作，静默放行会在依赖故障时移除唯一的共享限流边界。
...
- **选了哪个 + 为什么**：草案选择 `IDEMPOTENCY_OUTCOME_UNKNOWN` 并待用户批准，因为仅凭 Redis 写失败无法跨事务与外部副作用证明 operation 未提交。
...
- **选了哪个 + 为什么**：草案选择 503 且无内存 fallback 并待用户批准，因为 unknown 不能伪装成 not-found，多实例内存状态也不一致。
...
- **选了哪个 + 为什么**：草案选择 503 且不改 DTO 并待用户批准，因为零是业务事实而不是故障占位，同时避免 C4c 扩成统计 API schema 重设计。
...
- **选了哪个 + 为什么**：草案选择 mock 加隔离 Testcontainers 并待用户批准，以同时锁定分支逻辑和真实连接中断语义且不破坏用户环境。
- **放弃的代价**：验证耗时更长并依赖 Docker；首次运行可能需要下载既有基础设施镜像。

### DR-9：C4c 是否包含任务恢复

- **面临的选择**：在 Redis 故障语义中同时实现任务重放/协调，或只禁止假成功并把恢复留给 C5。
- **选了哪个 + 为什么**：草案选择只锁定故障结果并待用户批准，因为持久输入、孤儿协调与续跑已有 C5a/C5b 顺序，提前混入会越过依赖边界。
