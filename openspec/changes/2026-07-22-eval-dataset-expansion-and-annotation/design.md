...
- **选了哪个 + 为什么**：建议 150；比 100 有更好的类别交叉覆盖，又能把新增量控制在 120 条，语义复核仍可执行，待用户在事前闸门确认。
...
- **选了哪个 + 为什么**：建议完整保留并只追加；这样 C7/C8a 身份可审计，C8b 只改变覆盖规模，待用户在事前闸门确认。
...
- **选了哪个 + 为什么**：建议首版只用现有 3 份 fixture；控制变量并避免同时重建 KB/corpus，待用户在事前闸门确认。
...
- **选了哪个 + 为什么**：建议定向补弱项；保留事实/定义覆盖，同时显著增加 reasoning、multi-hop 和 no-answer，待用户在事前闸门确认。
...
- **选了哪个 + 为什么**：建议固定交叉矩阵；它直接打破现有 type 与 difficulty 强耦合，待用户在事前闸门确认。
...
- **选了哪个 + 为什么**：建议区间门槛；允许 multi-hop 多来源和文档内容差异，同时避免某一文档主导，待用户在事前闸门确认。
...
- **选了哪个 + 为什么**：建议不新增 split；项目当前不训练模型，隐藏 benchmark 也无法由 tracked 数据实现，避免无真实用途的 schema bump，待用户在事前闸门确认。
...
- **选了哪个 + 为什么**：建议 sample schema 保持 v1、review sidecar；评测输入和治理证据职责分开，待用户在事前闸门确认。
...
- **选了哪个 + 为什么**：建议显式多版本并延迟切默认；当前 checkout 可验证旧 release，回滚也清晰，待用户在事前闸门确认。
...
- **选了哪个 + 为什么**：建议机器可读 sidecar；能检查 150 条是否全审、避免复制问题正文，也不改 sample schema，待用户在事前闸门确认。
...
- **选了哪个 + 为什么**：建议升级 manifest schema v2；正式 release 的 review evidence 也应有 path/hash/bytes/count 身份，待用户在事前闸门确认。
- **放弃的代价**：不进 manifest 会让 review 与 release 漂移；添加未校验字段只是装饰性元数据。

### 决策 12：新增 ID 分配
- **面临的选择**：全局重新编号；随机 UUID；按现有 type prefix 从最大序号继续追加。
- **选了哪个 + 为什么**：建议延续 type prefix；人工审查可读并与 schema v1 兼容，待用户在事前闸门确认。
...
- **选了哪个 + 为什么**：建议确定性 exact grounding；可离线复核，不依赖 embedding/LLM，待用户在事前闸门确认。
...
- **选了哪个 + 为什么**：建议两个独立证据点；在固定 3 文档 corpus 内可执行，又不会为了跨文档硬造问题，待用户在事前闸门确认。
...
- **选了哪个 + 为什么**：建议相邻域 hard negative + 全 corpus 复核；更接近真实拒答压力且不依赖刻意乱码，待用户在事前闸门确认。
