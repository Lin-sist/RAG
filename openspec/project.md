...
- 当前默认评测 release 仍是已验收的 30 条 `rag-eval-dev-v1`；C8b 已离线构建待验收的 150 条 `rag-eval-dev-v2`（30 条 immutable seed + 120 条新增），具备 quota、grounding、duplicate 与 review sidecar 门禁，但仍不是生产数据集或隐藏 benchmark。
...
- C8b 正在待验收：显式 v2 manifest 固定 150 条 exact quota、v1 seed raw/object/order identity、103 条新增 answerable 的 242 个 fixture contexts、150 条 review records 与三份 fixture coverage 49/43/44；默认 manifest 仍指向 v1，C8b delta 尚未接受、change 尚未归档。
