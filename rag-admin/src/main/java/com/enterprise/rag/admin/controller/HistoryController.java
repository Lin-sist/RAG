...
public class HistoryController {
    ...
    public ResponseEntity<ApiResponse<PageResult<QAHistoryDTO>>> getHistory(
            @Parameter(description = "知识库 ID（可选）")
    ...
    @ApiResponses(value = {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "200",
            description = "获取成功",
            content = @Content(schema = @Schema(implementation = QAHistoryDTO.class))
        ),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "404",
            description = "历史记录不存在"
    ...
    public ResponseEntity<ApiResponse<QAHistoryDTO>> getHistoryById(
            @Parameter(description = "历史记录 ID", required = true)
            @PathVariable Long id) {
        log.debug("获取问答历史详情: id={}", id);
        return qaHistoryService.getById(id)
                .map(history -> ResponseEntity.ok(ApiResponse.success(history)))
                .orElseThrow(() -> new BusinessException("HISTORY_001", "历史记录不存在: " + id));
    }
    ...
    @ApiResponses(value = {
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "201",
            description = "反馈提交成功",
            content = @Content(schema = @Schema(implementation = QAFeedbackDTO.class))
        ),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "400",
            description = "请求参数错误或已提交过反馈"
        ),
        @io.swagger.v3.oas.annotations.responses.ApiResponse(
            responseCode = "404",
            description = "历史记录不存在"
