...
- 阶段：C15 实现中（Read-Only Resources / discovery checkpoint）
...
1. 用户已于 2026-07-28 要求启动 C15 规划实现，视为批准 proposal、初始 18 条 design decisions、tasks、7 requirements / 26 scenarios delta，以及规划内固定版本官方 SDK / conformance 工具引入与下载。SDK 实测后补充的决策 19 随本 discovery checkpoint 待用户复核，复核前不进入 Resource read。
...
3. authentication/Origin/local-only/request-size/request identity checkpoint 已提交为 `e17adca`，URI parser 已提交为 `74c9355`；authenticated `resources/templates/list` 与 `resources/list` discovery 已完成聚焦实现，下一纵向切片在用户复核决策 19 后进入 KB Resource read，不横向预写 document/chunk read 或 Tools。
...
- 当前 C15 已加入固定官方 SDK 2.0.0、default-off `/mcp`、sessionless initialize、现有 JWT 逐请求保护、exact Origin/local-only/request-size guard、server-derived transport identity、严格 KB/document/chunk Resource URI parser、固定三模板与按当前身份重新授权的 KB Resource 分页列表；Resource read、Tools、side-effect、双 tenant、conformance/client evidence 尚未实现。
