...
- 阶段：C15 实现中（transport + authentication/exposure checkpoint）
...
3. authentication/Origin/local-only/request-size/request identity 已形成首个安全 checkpoint；下一纵向切片从 Resource URI parser 的恶意输入 RED 开始，不横向预写 Tools。
...
- 当前 C15 已加入固定官方 SDK 2.0.0、default-off `/mcp`、sessionless initialize、现有 JWT 逐请求保护、exact Origin/local-only/request-size guard 与 server-derived transport identity；Resources/Tools、side-effect、双 tenant、conformance/client evidence 尚未实现。
